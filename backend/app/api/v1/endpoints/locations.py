...
async def create_location(data: LocationCreate, db=Depends(get_db), current_user=Depends(get_current_user)):
    base_code = data.code.strip().upper() if data.code else _to_code(data.name)
    if not base_code:
        base_code = str(uuid.uuid4())[:8].upper()
    # Tự động thêm hậu tố _2, _3... nếu mã bị trùng
    code = base_code
    suffix = 2
    while (await db.execute(select(Location).where(Location.code == code))).scalar_one_or_none():
